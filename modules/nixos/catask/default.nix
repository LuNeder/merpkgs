...
let
  defaultPkg = 
    if pkgs ? merpkgs && pkgs.merpkgs ? catask then pkgs.merpkgs.catask
    else null;
  cfg = config.merpkgs.services.catask;
...
in
{
  options = {
    merpkgs.services.catask = {
      ...
      package = lib.mkOption {
        ...
        default = defaultPkg;
        description = "The catask package to use.";
      };
  ...
  config = lib.mkIf cfg.enable {
    assertions = [{
      ...
      message = "catask package not found. Please add the merpkgs overlay or set services.catask.package manually.";
    }];
